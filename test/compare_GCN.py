...
RESULT_DIR = "results/plots_GCN"
os.makedirs(RESULT_DIR, exist_ok=True)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# =====================================================
# Load GCN encoder (Lightning-safe)
# =====================================================
...
state = torch.load(
    "results/models/final_model_GCN.ckpt",
    map_location=device,
)

# If checkpoint was saved as plain state_dict
...
# Featurization (FIXED)
# =====================================================
def featurize_split(dc_dataset):
    df = dc_dataset.to_dataframe()
    df = df.rename(columns={"X": "smiles"})

    # ---- Normalize to SMILES STRING (CRITICAL FIX) ----
    def to_smiles(x):
    ...
    df["smiles"] = df["smiles"].apply(to_smiles)
    ...
    targets = df.filter(regex="^y").values
    ...
    with torch.no_grad():
        gcn_emb = np.concatenate(
            [
                model.get_graph_embedding(batch.to(device)).cpu().numpy()
                for batch in loader
            ],
            axis=0,
        )
...
for name, loader_fn, task_type in BENCHMARKS:
    ...
    y_train, X_train = featurize_split(train_set)
    y_test, X_test = featurize_split(test_set)
...
print(f"\n✅ GCN benchmark results saved to {output_file}")
