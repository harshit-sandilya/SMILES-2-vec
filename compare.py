...
parser = ArgumentParser(
    description="Run MoleculeNet benchmark for a pre-trained GNN model."
)
parser.add_argument(
    "--model-file",
    type=str,
    required=True,
    help="Path to the final GNN model file (.pt or .pth)",
)
...
print(
    f"Model weights loaded successfully from {args.model_file} and running on {device}."
)
print("=" * 70)
...
def process_and_featurize_split(dc_dataset, tokenizer, max_atoms=64):
    ...
    with torch.no_grad():
        features["Our GNN"] = np.concatenate(
            [
    ...
    n_bits = 2048
    features["ECFP"] = np.array(
        [
            (
                fp
                if (fp := generate_ecfp(m, n_bits=n_bits)) is not None
                else np.zeros(n_bits, dtype=np.int8)
            )
            for m in mols
        ]
    )
    features["MACCS"] = np.array(
        [
            (
                fp
                if (fp := generate_maccs(m)) is not None
                else np.zeros(166, dtype=np.int8)
            )
            for m in mols
        ]
    )
    features["RDKit FP"] = np.array(
        [
            (
                fp
                if (fp := generate_rdkit_fp(m, n_bits=n_bits)) is not None
                else np.zeros(n_bits, dtype=np.int8)
            )
            for m in mols
        ]
    )
    features["Atom Pairs"] = np.array(
        [
            (
                fp
                if (fp := generate_atom_pairs(m, n_bits=n_bits)) is not None
                else np.zeros(n_bits, dtype=np.int8)
            )
            for m in mols
        ]
    ...
    for task_name in y_train_df.columns:
        ...
        for method_name, X_train in X_train_features.items():
            ...
            if metrics:
                ...
                all_results.append(
                    {
                        "Dataset": f"{config['name']}-{task_name}",
                        "Method": method_name,
                        **metrics,
                    }
                )
...
available_methods = [
    m for m in method_order if m in final_table.columns.get_level_values("Method")
]
